...
public class PlowLeftSideBlueCommand extends SequentialCommandGroup {
    Trajectory trajectory1, trajectory2, trajectory3;
    public PlowLeftSideBlueCommand(CANDrivetrain m_drivetrain, CANLauncher m_launcher){
        ...
        interiorWayPoint.add(new Translation2d(0,2));

        //trajectory 2
        Pose2d turningPoint = new Pose2d(-.7,.5, Rotation2d.fromDegrees(22.476));
        var interiorWaypoints1 = new ArrayList<Translation2d>();
        interiorWaypoints1.add(new Translation2d(-.5,.2));

        //trajectory 3
        Pose2d endpoint = new Pose2d(-.9,-1.3, Rotation2d.fromDegrees(0));
        
        var interiorWaypoints2 = new ArrayList<Translation2d>();
        interiorWaypoints2.add(new Translation2d(-.9,-.7));

        trajectory1 = TrajectoryGenerator.generateTrajectory(startingPoint, interiorWayPoint, ShootPoint, config);
        trajectory2 = TrajectoryGenerator.generateTrajectory(ShootPoint, interiorWaypoints1, turningPoint, config);
        trajectory3 = TrajectoryGenerator.generateTrajectory(turningPoint, interiorWaypoints2, endpoint, config2);
